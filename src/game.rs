...
impl Game {
    ...
    fn save_settings(&mut self) {
        ...
        self.settings.camera.mouse_sensitivity = self.camera.sensitivity;
        self.settings.camera.invert_x = self.camera.invert_x;
        self.settings.camera.invert_y = self.camera.invert_y;
        self.settings.save();
    }

    fn cycle_resolution(&mut self, direction: i32) {
        let list = self.renderer.supported_resolutions();
        if list.is_empty() {
            return;
        }
        ...
        let current = list
            .iter()
            .position(|&(w, h)| {
                w == self.settings.graphics.resolution_width
    ...
    fn cycle_msaa(&mut self, direction: i32) {
        let options = [0u8, 2, 4, 8];
        let current = options
            .iter()
    ...
    fn cycle_display(&mut self, direction: i32) {
        match self.menu_selected {
            0 => {
                let current = self.settings.graphics.texture_filtering as i32;
                self.settings.graphics.texture_filtering =
                    ((current + direction).rem_euclid(3)) as u8;
            }
            1 => self.cycle_msaa(direction),
            2 => self.cycle_resolution(direction),
            3 => {
                let current = self.settings.graphics.fullscreen_mode as i32;
                self.settings.graphics.fullscreen_mode =
                    ((current + direction).rem_euclid(3)) as u8;
            }
            4 => {
                if direction != 0 {
    ...
    fn cycle_controls(&mut self, direction: i32) {
        match self.menu_selected {
            0 => {
                self.camera.sensitivity =
                    (self.camera.sensitivity + 0.001 * direction as f32).clamp(0.001, 0.02);
            }
            1 => {
                if direction != 0 {
                    self.camera.invert_x = !self.camera.invert_x;
                }
            }
            2 => {
                if direction != 0 {
                    self.camera.invert_y = !self.camera.invert_y;
                }
            }
            _ => {}
        }
    ...
    fn activate_menu(&mut self) {
        match self.menu {
            Some(Menu::Pause) => match self.menu_selected {
                0 => self.resume(),
                1 => {
                    self.menu = Some(Menu::Settings);
                    self.menu_selected = 0;
                }
                2 => std::process::exit(0),
                _ => {}
            },
            Some(Menu::Settings) => match self.menu_selected {
                0 => {
                    self.menu = Some(Menu::Controls);
                    self.menu_selected = 0;
                }
                1 => {
                    self.menu = Some(Menu::Display);
                    self.menu_selected = 0;
                }
                2 => self.open_pause(),
                _ => {}
            ...
            Some(Menu::Display) => match self.menu_selected {
    ...
    fn menu_back(&mut self) {
        match self.menu {
            Some(Menu::Pause) => self.resume(),
            Some(Menu::Settings) => self.open_pause(),
            Some(Menu::Controls) => {
                self.menu = Some(Menu::Settings);
                self.menu_selected = 0;
            }
    ...
    pub fn handle_key(&mut self, event: KeyEvent) {
        ...
        if let PhysicalKey::Code(code) = event.physical_key {
            if self.menu.is_some() {
                if pressed {
                    match code {
                        KeyCode::Escape => self.menu_back(),
                        KeyCode::ArrowUp => {
                            let count = match self.menu {
                                Some(Menu::Pause) => PAUSE_COUNT,
                                Some(Menu::Settings) => SETTINGS_COUNT,
                                Some(Menu::Controls) => CONTROLS_COUNT,
                                Some(Menu::Display) => DISPLAY_COUNT,
                                None => 1,
                            };
                            self.menu_selected = (self.menu_selected + count - 1) % count;
                        }
                        KeyCode::ArrowDown => {
                            let count = match self.menu {
                                Some(Menu::Pause) => PAUSE_COUNT,
                        ...
                        KeyCode::ArrowLeft => match self.menu {
                            Some(Menu::Controls) => self.cycle_controls(-1),
                            Some(Menu::Display) => self.cycle_display(-1),
                            _ => {}
                        },
                        KeyCode::ArrowRight => match self.menu {
                            Some(Menu::Controls) => self.cycle_controls(1),
                            Some(Menu::Display) => self.cycle_display(1),
                            _ => {}
                        },
                        KeyCode::Enter | KeyCode::Space => self.activate_menu(),
                        _ => {}
                    }
                }
                return;
            ...
            match code {
                KeyCode::Escape if pressed => self.open_pause(),
    ...
    pub fn handle_cursor_moved(&mut self, x: f64, y: f64) {
        if !self.mouse_captured || self.menu.is_some() {
            return;
        }

        let (cx, cy) = self.renderer.window_center();
        let dx = x - cx;
        let dy = y - cy;
        ...
        self.camera.rotate(
            dx as f32 * self.camera.sensitivity,
            dy as f32 * self.camera.sensitivity,
        );
    ...
    pub fn handle_mouse_click(&mut self, state: ElementState, button: MouseButton) {
        ...
        match button {
            MouseButton::Right => {
                ...
                if self.aiming && !self.mouse_captured {
                    self.set_mouse_capture(true);
                }
            }
            MouseButton::Left => {
    ...
    pub fn update_and_render(&mut self) {
        ...
        self.renderer.render(
            ...
            self.settings.graphics.texture_filtering,
            self.settings.graphics.msaa_samples,
            self.settings.graphics.fullscreen_mode,
            self.settings.graphics.resolution_width,
            self.settings.graphics.resolution_height,
            self.aiming,
    ...
    fn update(&mut self, dt: f32) {
        ...
        let speed = if self.input_state.sprint {
            SPRINT_SPEED
        } else {
            WALK_SPEED
        };
        ...
        if movement.length_squared() > 0.0001 {
            ...
            let target_rotation = if self.aiming {
        ...
        if self.input_state.jump && !self.player.is_jumping && !self.player.is_falling {
            self.player.velocity.y = 8.0;
            self.player.is_jumping = true;
        }

        self.physics_engine
            .update(&mut self.player, &self.world, movement, dt);
        self.camera
            .follow(self.player.position + Vec3::new(0.0, 1.0, 0.0));

        let target_distance = if self.aiming {
            AIM_CAMERA_DISTANCE
        } else {
            NORMAL_CAMERA_DISTANCE
        };

        self.camera.distance +=
            (target_distance - self.camera.distance)
                * (1.0 - (-AIM_CAMERA_SMOOTHNESS * dt).exp());
