...
impl PhysicsEngine {
    pub fn new(asset_manager: &AssetManager) -> Self {
        // The player's collision shape is authored as an asset just like every
        // world object's collision shape. Physics no longer has a hardcoded
        // player bounding box.
    ...
    pub fn update(&mut self, player: &mut Player, world: &World, movement: Vec3, dt: f32) {
        ...
        // There is deliberately no artificial ground plane or +/-500 boundary.
        // Leaving the authored collision geometry means the player actually
        // leaves the map instead of floating on an invisible floor.
    ...
    fn player_ground_y(&self) -> f32 {
        // The authored player collision model defines where the feet are.
