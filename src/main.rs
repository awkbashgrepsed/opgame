...
fn main() {
    ...
    // Start in a normal window. Renderer::new() will create the GL context,
    // and the saved display mode is applied once the event loop is running.
    let window_builder = WindowBuilder::new()
        .with_title("OPGAME - OpenGL 2.1+")
        .with_inner_size(winit::dpi::LogicalSize::new(
            settings.graphics.resolution_width as f64,
            settings.graphics.resolution_height as f64,
        ));
    ...
    let (window, gl_config) = DisplayBuilder::new()
        ...
        .build(&event_loop, template, |configs| {
            configs
                .max_by_key(|config| {
                    if requested_msaa == 0 && config.num_samples() == 0 {
                        10_000
                    } else if config.num_samples() == requested_msaa {
                        20_000
                    } else {
    ...
    log::info!(
        "Requested MSAA {}x, selected OpenGL config with {} samples",
        requested_msaa,
