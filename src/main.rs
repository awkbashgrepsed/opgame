...
fn main() {
    ...
    // Fullscreen is intentionally applied after the event loop starts. Winit
    // queues window mutations until the event-loop thread is processing events;
    // applying Exclusive before that point can be lost during GL initialization.
