...
unsafe fn load_texture(image: &gltf::image::Data) -> u32 {
    ...
    match image.format {
        gltf::image::Format::R8 => for &r in &image.pixels { rgba.extend_from_slice(&[r, r, r, 255]); },
        gltf::image::Format::R8G8 => for p in image.pixels.chunks_exact(2) { rgba.extend_from_slice(&[p[0], p[0], p[0], p[1]]); },
        gltf::image::Format::R8G8B8 => for p in image.pixels.chunks_exact(3) { rgba.extend_from_slice(&[p[0], p[1], p[2], 255]); },
