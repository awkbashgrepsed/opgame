...
unsafe fn load_model(relative_path: &str, label: &str) -> Vec<ModelMesh> {
    ...
    log::info!(
        "Loaded {label} from {}: {} mesh primitive(s), {} texture(s)",
        path.display(),
        meshes.len(),
        texture_cache.len()
    );
    ...
    if let Some(mesh) = node.mesh() {
        for primitive in mesh.primitives() {
            ...
            let normals: Vec<Vec3> = match reader.read_normals() {
                Some(values) => values
                    .map(|n| {
                        let normal = normal_matrix * Vec4::new(n[0], n[1], n[2], 0.0);
                        Vec3::new(normal.x, normal.y, normal.z).normalize_or_zero()
                    })
                    .collect(),
...
fn load_player_model() -> &'static Vec<ModelMesh> {
    PLAYER_MODEL.get_or_init(|| unsafe { load_model("models/player.glb", "player") })
}
