...
static TEXTURE_FILTERING: std::sync::atomic::AtomicU8 =
    std::sync::atomic::AtomicU8::new(0);

pub fn set_texture_filtering(mode: u8) {
    TEXTURE_FILTERING.store(mode.min(2), std::sync::atomic::Ordering::Relaxed);
}

unsafe fn apply_texture_filtering() {
    let mode = TEXTURE_FILTERING.load(std::sync::atomic::Ordering::Relaxed);
    let (min, mag) = match mode {
        1 => (gl::LINEAR_MIPMAP_LINEAR, gl::LINEAR),
        2 => (gl::NEAREST, gl::NEAREST),
        _ => (gl::LINEAR_MIPMAP_NEAREST, gl::LINEAR),
    };
...
unsafe fn load_texture(image: &gltf::image::Data) -> u32 {
    ...
    gl::TexParameteri(gl::TEXTURE_2D, GL_GENERATE_MIPMAP, 1);
...
unsafe fn load_node(
    ...
    texture_cache: &mut HashMap<usize, u32>,
    ...
    if let Some(mesh) = node.mesh() {
        for primitive in mesh.primitives() {
            ...
            let texture = pbr.base_color_texture().map(|info| {
                ...
                if let Some(&texture) = texture_cache.get(&i) {
                    texture
                } else {
                    let texture = load_texture(&images[i]);
                    texture_cache.insert(i, texture);
                    texture
                }
            });
...
static ASSET_MANAGER: OnceLock<assets::AssetManager> = OnceLock::new();

fn asset_manager() -> &'static assets::AssetManager {
    ASSET_MANAGER.get_or_init(assets::AssetManager::load)
}

fn load_asset_model(id: &str) -> &'static Vec<ModelMesh> {
    let manager = asset_manager();
    let path = manager.model_path(id);
    let key = path.to_string_lossy().to_string();
    let cache = ASSET_MODELS.get_or_init(|| Mutex::new(HashMap::new()));
    let mut guard = cache.lock().expect("asset model cache poisoned");

    let label = format!("asset '{id}'");
    let meshes = guard
        .entry(key.clone())
        .or_insert_with(|| unsafe { load_model(&key, &label) });

    // The cache owns the model for the lifetime of the game.
    // This reference is safe because ASSET_MODELS is never dropped or moved.
    unsafe { &*(meshes as *const Vec<ModelMesh>) }
}
    ...
    for mesh in meshes {
        ...
        for &index in &mesh.indices {
            ...
            if let (Some(position), Some(normal)) =
                (mesh.positions.get(i), mesh.normals.get(i))
            {
                gl::Color4f(
                    mesh.color[0],
                    mesh.color[1],
...
pub unsafe fn draw_player(position: Vec3, rotation: f32) {
    let meshes = load_asset_model("player");
...
pub unsafe fn draw_map() {
    let meshes = load_asset_model("map");
    let scale = asset_manager().default_scale("map");
...
pub unsafe fn draw_asset(id: &str, position: Vec3, rotation: Vec3, scale: Vec3) {
    let meshes = load_asset_model(id);

    gl::PushMatrix();
    gl::Translatef(position.x, position.y, position.z);
    gl::Rotatef(rotation.x.to_degrees(), 1.0, 0.0, 0.0);
    gl::Rotatef(rotation.y.to_degrees(), 0.0, 1.0, 0.0);
    gl::Rotatef(rotation.z.to_degrees(), 0.0, 0.0, 1.0);
    gl::Scalef(scale.x, scale.y, scale.z);
    draw_meshes(meshes);
    gl::PopMatrix();
}
